...
static u8 *use_iface,                   /* Interface to listen on             */
          ...
          *device_logfile;             /* Device log file name               */
...
static FILE* devicelf;                        /* Log file stream                    */
...
"Usage: vcs-device-identity [ ...options... ] [ 'filter rule' ]\n"
"\n"
...
static void open_device_log(void) {

  struct stat st;
  s32 devicelog_fd;

  devicelog_fd = open((char*)device_logfile, O_WRONLY | O_APPEND | O_NOFOLLOW | O_LARGEFILE);

  if (devicelog_fd >= 0) {

    if (fstat(devicelog_fd, &st)) PFATAL("fstat() on '%s' failed.", device_logfile);

    if (!S_ISREG(st.st_mode)) FATAL("'%s' is not a regular file.", device_logfile);

  } else {

    if (errno != ENOENT) PFATAL("Cannot open '%s'.", device_logfile);

    devicelog_fd = open((char*)device_logfile, O_WRONLY | O_CREAT | O_EXCL | O_NOFOLLOW,
                  LOG_MODE);

    if (devicelog_fd < 0) PFATAL("Cannot open '%s'.", device_logfile);

  }

  if (flock(devicelog_fd, LOCK_EX | LOCK_NB))
    FATAL("'%s' is being used by another process.", device_logfile);

  devicelf = fdopen(devicelog_fd, "a");

  if (!devicelf) FATAL("fdopen() on '%s' failed.", device_logfile);

  SAYF("[+] Log device file '%s' opened for writing.\n", device_logfile);
...
int main(int argc, char** argv) {
  ...
  while ((r = getopt(argc, argv, "+LS:df:i:m:o:pr:s:t:u:x:")) != -1) switch (r) {
  ...
  if (device_logfile) open_device_log();
  ...
  //vcs
  //pthread_t thread_id;
  //pthread_create(&thread_id, NULL, watch_dhcpleasefile, NULL);
